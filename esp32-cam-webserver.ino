...
/* Settings for OTA to work

Board: "ESP32 Dev Module ,
Partition Scheme: "Minimal SPIFFS (1.9MB APP with OTA/190KB SPIFFS)
and PSRAM: "Enabled

https://community.platformio.org/t/esp32-cam-ota-upload-problem/17179/9
...
// water counter
char* device_id="water_counter";
char* bmax="http://192.168.1.104:8889";
const int sensor=2; // IO02
const int changeLed=15;
const int IRLeds=12;
...
void processSensor() {
  static int lastValue=digitalRead(sensor);
  static unsigned long totalCounter = 0;
  ...
  if (sensorValue != lastValue) {
    ...
    // sensor changes twice on each turn
    if (sensorValue == 1 && totalCounter%2 != 0){
        counter++;
  ...
  if (millis() - lastSent > 60*1000) {
        epoch = getEpochTime();
        if (!epoch) {
          Serial.println("Error getting epoch time");
          return;
        }
        
        // sensor status changes twice on each turn
        sprintf(buffer, "add/%d:%d", epoch, counter);
